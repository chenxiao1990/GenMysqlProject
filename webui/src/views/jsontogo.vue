<template>
  <div>
    <div style="width:33%; float:left;">
      json:
      <el-input type="textarea" style="color: #000;" rows="25" placeholder="请输入内容" v-model="jsonstr" show-word-limit></el-input>
    </div>
    <div style="width:33%; float:left;color:#000;">
      gostruct 内联:
      <el-input type="textarea" style="color: #000;" rows="25" placeholder="请输入内容" v-model="gocode" show-word-limit></el-input>
    </div>
    <div style="width:33%; float:left;color:#000;">
      gostruct 分离:
      <el-input type="textarea" style="color: #000;" rows="25" placeholder="请输入内容" v-model="gocode2" show-word-limit></el-input>
    </div>
  </div>
</template>
<script>
export default {
  ...
  created() {
      this.interval = setInterval(()=>{
          this.gocode = this.jsonToGo(this.jsonstr,"",false).go;
          this.gocode2 = this.jsonToGo(this.jsonstr,"",true).go;
      },500);
  },
  destroyed() {
...
*/
    ...
    jsonToGo(json, typename, flatten  ) {
      let data;
      ...
      function goType(val) {
        ...
        switch (typeof val) {
          case "string":
            if (
              /\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(\.\d+)?(\+\d\d:\d\d|Z)/.test(val)
            )
              return "time.Time";
            else return "string";
